...
export async function POST(req: Request) {
  ...
  const requestsInWindow = userRequests.filter(ts => now - ts < RATE_LIMIT_WINDOW);
  ...
  if (userMessage.toLowerCase().includes('error test')) {
    const stream = new ReadableStream({
      async start(controller) {
        ...
        send('{"type": "error", "error": "Failed to stamp hash: Insufficient funds."}\n');
        controller.close();
      },
    });
  ...
  const stream = new ReadableStream({
    async start(controller) {
      ...
      send('{"type": "step_result", "result": "✅ Hash stamped successfully! Transaction ID: 0x123...def"}\n');
      ...
      await new Promise(res => setTimeout(res, 500));
            ...
            send('{"type": "final_response", "response": "Your document has been successfully stamped and verified on the blockchain.", "proof": { "timestamp": "2025-08-29T12:00:00Z", "hash": "...", "transactionId": "0x123...def" }}');
