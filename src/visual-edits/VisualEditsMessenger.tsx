...
export const CHANNEL = "IntegritasS_HOVER_v1" as const;
const VISUAL_EDIT_MODE_KEY = "Integritass_visual_edit_mode" as const;
const FOCUSED_ELEMENT_KEY = "Integritass_focused_element" as const;

// Deduplicate helper for high-frequency traffic (HIT / FOCUS_MOVED / SCROLL)
// -----------------------------------------------------------------------------
let _IntegritassLastMsg = "";
const postMessageDedup = (data: any) => {
  try {
    const key = JSON.stringify(data);
    if (key === _IntegritassLastMsg) return; // identical – drop
    _IntegritassLastMsg = key;
  } catch {
...
// Helper to parse data-Integritass-id to extract file path, line, and column
const parseIntegritassId = (
  IntegritassId: string
): { filePath: string; line: number; column: number } | null => {
  // Format: "filepath:line:column"
  const parts = IntegritassId.split(":");
...
export default function HoverReceiver() {
  ...
  useEffect(() => {
    if (isVisualEditMode && !styleElementRef.current) {
      ...
      style.textContent = `
        ...
        [data-Integritass-protected="true"] {
          user-select: none !important;
          -webkit-user-select: none !important;
          -moz-user-select: none !important;
          -ms-user-select: none !important;
  ...
  const protectChildElements = (element: HTMLElement) => {
    ...
    childElements.forEach((child) => {
      ...
      childEl.setAttribute("data-Integritass-protected", "true");
  ...
  const restoreChildElements = (element: HTMLElement) => {
    ...
    protectedElements.forEach((child) => {
      ...
      childEl.removeAttribute("data-Integritass-protected");
  ...
  const handleTextChange = (element: HTMLElement) => {
    ...
    // Get the Integritass ID from the element to ensure we're working with the right one
    const IntegritassId = element.getAttribute("data-Integritass-id");
    if (!IntegritassId) return;
    ...
    if (newText !== oldText) {
      const parsed = parseIntegritassId(IntegritassId);
      if (!parsed) return;
  ...
  const handleStyleChange = (
    ...
    // Find ALL elements with the same Integritass ID
    const allMatchingElements = document.querySelectorAll(
      `[data-Integritass-id="${IntegritassId}"]`
    ...
    const existingStyles = appliedStylesRef.current.get(IntegritassId) || {};
    appliedStylesRef.current.set(IntegritassId, {
      ...existingStyles,
      ...styles,
    });
  ...
  const handleStyleBlur = (element: HTMLElement) => {
    ...
    const appliedStyles = appliedStylesRef.current.get(IntegritassId);
  ...
  const flushImageSrcChange = () => {
    ...
    const IntegritassId = imgElement.getAttribute("data-Integritass-id");
    if (!IntegritassId) return;
  ...
  useEffect(() => {
    function handleMessage(e: MessageEvent) {
      if (e.data?.type === "IntegritasS_STYLE_UPDATE") {
        const { elementId, styles } = e.data;

        // Find ALL elements with the same Integritass ID
        const allMatchingElements = document.querySelectorAll(
          `[data-Integritass-id="${elementId}"]`
        ) as NodeListOf<HTMLElement>;
      ...
      } else if (e.data?.type === "IntegritasS_IMAGE_UPDATE") {
  ...
  useEffect(() => {
    ...
    const handleMouseUp = () => {
      if (focusedElementRef.current && focusedElementId) {
        ...
        // Extract file info from data-Integritass-id
        const IntegritassId = element.getAttribute("data-Integritass-id");
        if (IntegritassId) {
          const parsed = parseIntegritassId(IntegritassId);
          if (parsed) {
  ...
  useEffect(() => {
    ...
    function onPointerMove(e: PointerEvent) {
      ...
      const hit =
        document
          ...
          ?.closest<HTMLElement>("[data-Integritass-id]") ?? null;
      ...
      if (hit !== lastHitElementRef.current) {
        ...
        const hitId = hit.getAttribute("data-Integritass-id");
        ...
        const tagName =
          hit.getAttribute("data-Integritass-name") ||
          hit.tagName.toLowerCase();

        // Update hover boxes immediately for instant feedback
        // Find ALL elements with the same Integritass ID
        const allMatchingElements = document.querySelectorAll(
          `[data-Integritass-id="${hitId}"]`
        ) as NodeListOf<HTMLElement>;
        ...
        allMatchingElements.forEach((element) => {
          ...
          const elementId = element.getAttribute("data-Integritass-id");
    ...
    function onClickCapture(e: MouseEvent) {
      ...
      const target = e.target as HTMLElement;
      const hit = target.closest<HTMLElement>("[data-Integritass-id]");
      ...
      if (hit) {
        ...
        // Find ALL other elements with the same Integritass ID and show hover boxes
        const allMatchingElements = document.querySelectorAll(
          `[data-Integritass-id="${hitId}"]`
        ) as NodeListOf<HTMLElement>;
        ...
        // Clicked on empty space or element without data-Integritass-id
    ...
    function onMsg(e: MessageEvent<ParentToChild>) {
      ...
      if (e.data.msg === "CLEAR_INLINE_STYLES" && "elementId" in e.data) {
        // Find ALL elements with the same Integritass ID
        const allMatchingElements = document.querySelectorAll(
          `[data-Integritass-id="${e.data.elementId}"]`
        ) as NodeListOf<HTMLElement>;
      ...
      if (e.data.msg === "SHOW_ELEMENT_HOVER" && "elementId" in e.data) {
        ...
        if (allMatchingElements.length > 0) {
          ...
          allMatchingElements.forEach((element) => {
            ...
            if (!tagName) {
              tagName =
                element.getAttribute("data-Integritass-name") ||
