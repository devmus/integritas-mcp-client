  ...
  try {
    ...
    (0, estree_walker_1.walk)(ast, {
      enter(node) {
        ...
        let IntegritassId = `${rel}:${line}:${column}`;
        // Enhance the ID with context if we have map information
        if (mapContext) {
          IntegritassId += `@${mapContext.arrayName}`;
        }
        ...
        ms.appendLeft(
          node.name.end,
          ` data-Integritass-id="${IntegritassId}" data-Integritass-name="${semanticName}"`
        );
        mutated = true;
      },
    });
